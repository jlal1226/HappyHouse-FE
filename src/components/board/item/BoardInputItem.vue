...
export default {
  ...
  methods: {
    onSubmit(event) {
      ...
      let err = true;
      let msg = "";
      !this.article.user_id && ((msg = "작성자 입력해주세요"), (err = false), this.$refs.userid.focus());
      err && !this.article.title&& ((msg = "제목 입력해주세요"), (err = false), this.$refs.subject.focus());
    ...
    modifyArticle() {
      let param = {
        board_id: this.article.board_id,
        user_id: this.article.user_id,
        title: this.article.title,
        content: this.article.content,
      };
